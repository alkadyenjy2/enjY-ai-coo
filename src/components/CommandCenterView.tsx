    ...
    <div className="flex flex-col h-[calc(100vh-8rem)] bg-zinc-900 border border-zinc-800 rounded-2xl shadow-xl overflow-hidden text-zinc-200">
      <div className="bg-zinc-950 px-4 py-3 border-b border-zinc-800 flex items-center justify-between">
        ...
        <div className="text-right text-[11px] text-zinc-500 hidden sm:block font-mono">
          ...
          <div className="text-emerald-400 text-[10px]">Understand &rarr; Route &rarr; Execute &rarr; Verify &rarr; Report</div>
        </div>
      </div>
      ...
      <div className="border-b border-zinc-800 bg-zinc-950/80 px-4 py-3">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2 text-[10px] font-mono uppercase tracking-wider">
                ...
                <span className="text-emerald-400 font-bold">{isLoading ? 'EXECUTING' : routeSummary.status}</span>
              </div>
          ...
          <button
            ...
            className="shrink-0 px-2.5 py-1.5 rounded-lg border border-zinc-800 bg-zinc-900 hover:bg-zinc-800 text-[10px] font-mono text-zinc-300 transition-colors"
          >
        ...
        {showRouteDetails && (
          <div className="mt-3 grid grid-cols-1 sm:grid-cols-3 gap-2 text-[10px] font-mono">
            <div className="rounded-lg border border-zinc-800 bg-zinc-900 p-2.5">
              ...
              <div className="mt-1 text-emerald-400 font-bold">FREE FIRST</div>
            </div>
            <div className="rounded-lg border border-zinc-800 bg-zinc-900 p-2.5">
            ...
            <div className="rounded-lg border border-zinc-800 bg-zinc-900 p-2.5">
              ...
              <div className="mt-1 text-zinc-200 flex items-center gap-1.5"><ShieldCheck className="w-3 h-3 text-emerald-400" /> No paid route</div>
            </div>
            <div className="sm:col-span-3 rounded-lg border border-zinc-800 bg-zinc-950 p-2.5">
              <div className="text-zinc-500 uppercase tracking-wider mb-1">Fallback chain</div>
              <div className="flex flex-wrap gap-1.5">
                {routeSummary.fallbacks.map((name) => (
                  <span key={name} className="px-2 py-1 rounded-md bg-zinc-900 border border-zinc-800 text-zinc-300 flex items-center gap-1.5">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          return (
            <div key={msg.id} className={`flex gap-3 ${isAgent ? 'items-start' : 'items-start flex-row-reverse'}`}>
              ...
              <div className={`max-w-2xl space-y-2 ${isAgent ? 'text-left' : 'text-right'}`}>
                ...
                <div className={`p-3.5 rounded-2xl text-xs leading-relaxed font-sans ${isAgent ? 'bg-zinc-950 border border-zinc-800 text-zinc-100' : 'bg-zinc-100 text-zinc-950 font-medium'}`}>
                  ...
                  {msg.actionsTaken && msg.actionsTaken.length > 0 && (
                    <div className="mt-2.5 pt-2 border-t border-zinc-800 flex flex-wrap gap-1.5">
                      {msg.actionsTaken.map((act, i) => (
                        <span key={i} className="px-2 py-0.5 rounded text-[10px] font-mono bg-zinc-900 text-emerald-400 border border-emerald-500/30 flex items-center gap-1">
                ...
                {isAgent && msg.thoughtProcess && (
                  <div className="bg-zinc-950 border border-zinc-800 rounded-xl overflow-hidden text-[11px] font-mono">
                    <button onClick={() => toggleThought(msg.id)} className="w-full px-3 py-2 flex items-center justify-between text-zinc-400 hover:text-zinc-200 hover:bg-zinc-900 transition-colors">
                      <span className="font-semibold flex items-center gap-1.5 text-emerald-400"><Sparkles className="w-3.5 h-3.5" /> Execution Reasoning Loop (6 Steps)</span>
                    ...
                    {isThoughtExpanded && (
                      <div className="p-3 border-t border-zinc-800 space-y-2 font-mono text-[10px] text-zinc-300 bg-zinc-950">
                        {msg.thoughtProcess.understand && <div className="flex gap-2"><span className="text-emerald-400 font-bold shrink-0">1. UNDERSTAND:</span><span>{msg.thoughtProcess.understand}</span></div>}
                        ...
                        {msg.thoughtProcess.execute && <div className="flex gap-2"><span className="text-emerald-400 font-bold shrink-0">4. EXECUTE:</span><span>{msg.thoughtProcess.execute}</span></div>}
        ...
        {isLoading && (
          <div className="flex items-center gap-3 text-xs text-emerald-400 font-mono">
            <div className="w-8 h-8 rounded-xl bg-zinc-900 border border-zinc-800 flex items-center justify-center animate-spin"><RefreshCw className="w-4 h-4 text-emerald-400" /></div>
