...
test('MemPalace adapter real MCP smoke test', { timeout: 120_000 }, async (t) => {
  ...
  globalThis.fetch = async (...args) => { const response = await originalFetch(...args); if (String(args[0]).includes('/mcp')) { try { console.error('[MEMPALACE_RAW_HTTP]', await response.clone().text()); } catch {} } return response; };
  ...
  assert.equal(receipt.status, 'STORED'); assert.ok(receipt.memoryId);
  const ownRead = await gateway.read(receipt.memoryId, tenantA); assert.ok(ownRead); assert.match(ownRead.content, new RegExp(marker)); assert.equal(ownRead.tenantId, tenantA);
  const crossTenantRead = await gateway.read(receipt.memoryId, tenantB); assert.equal(crossTenantRead, null);
  const recall = await gateway.recall({ tenantId: tenantA, query: marker, limit: 5 }); assert.ok(recall.some((item) => item.memoryId === receipt.memoryId && item.content.includes(marker)));
  const crossTenantRecall = await gateway.recall({ tenantId: tenantB, query: marker, limit: 5 }); assert.equal(crossTenantRecall.length, 0);
  const provenance = await gateway.provenance(receipt.memoryId, tenantA); assert.ok(provenance); assert.equal(provenance.memoryId, receipt.memoryId); assert.equal(provenance.tenantId, tenantA);
  const forget = await gateway.forget({ memoryId: receipt.memoryId, tenantId: tenantA, reason: 'real MCP smoke cleanup', requestedBy: 'ci' }); assert.equal(forget.status, 'FORGOTTEN'); assert.equal(await gateway.read(receipt.memoryId, tenantA), null);
});
