...
test('persists Temporal worker records with signed tenant context and server secret', async () => {
  const originalFetch = globalThis.fetch;
  const originalUrl = process.env.SUPABASE_URL;
  ...
  globalThis.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
    requests.push({ url: String(input), init });
    return new Response(JSON.stringify([{ id: 'audit-temporal-1' }]), { status: 201 });
  }) as typeof fetch;
  ...
  try {
    ...
    assert.equal(body.organization_id, '11111111-1111-1111-1111-111111111111');
    assert.equal(body.user_id, '22222222-2222-2222-2222-222222222222');
    assert.equal((requests[0].init?.headers as Record<string, string>).Authorization, 'Bearer secret-test-key');
  } finally {
    globalThis.fetch = originalFetch;
    if (originalUrl === undefined) delete process.env.SUPABASE_URL;
    else process.env.SUPABASE_URL = originalUrl;
